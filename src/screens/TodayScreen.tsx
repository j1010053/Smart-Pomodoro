...
export function TodayScreen({ onNavigate }: { onNavigate: (page: AppPage) => void }) {
  ...
  return (
    <main className="app-shell">
      ...
      <section className="forecast-card"><div className="section-heading"><div><p className="eyebrow">未來七天</p><h2>截止前的每日安排</h2></div><span>{forecast.hasOverload ? "需要調整" : "可安排"}</span></div><div className="forecast-days">{forecast.days.map((day, index) => <div className={`forecast-day ${day.risk}`} key={day.date}><span>{index === 0 ? "今天" : `${new Date(`${day.date}T00:00:00`).getMonth() + 1}/${new Date(`${day.date}T00:00:00`).getDate()}`}</span><b>{day.scheduledMinutes}</b><small>／{day.capacityMinutes} 分</small></div>)}</div><p>{forecast.hasOverload ? `最早容量不足日：${forecast.firstRiskDate}。系統已排滿仍有工作無法在期限前完成。` : `系統會從今天起平均安排到截止日；彈性待辦 ${forecast.flexibleBacklogMinutes} 分鐘不列入期限壓力。`}</p></section>
